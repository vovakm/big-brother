...
class Users extends BB_Controller
{
	...
	public function userUpdate()
	{
		...
		if (intval($_REQUEST['id']) > 0)
			$id_account = intval($_REQUEST['id']); //find - int
		if (intval($_REQUEST['pass_num']) > 0)
			$data['number_pass_account'] = $_REQUEST['pass_num']; //edit - int
		else
...
//		$data['in_samba_account'] = $_REQUEST['login']; //system - no editable - bool
		...
		if (isset($_REQUEST['password'])) //edit - hash/SHA512 - str
			...
			if ($_REQUEST['id'] == '')
				$data['password_account'] = hash('sha512', $data['number_pass_account']);
		if ($_REQUEST['delete_image'] != 1)
		{
			...
			if (isset($_FILES['new_photo']) && strlen($_FILES['new_photo']['tmp_name']) > 0)
			{
				...
				if ($_FILES['new_photo']['error'] == UPLOAD_ERR_OK && is_uploaded_file($_FILES['new_photo']['tmp_name']))
					$image = file_get_contents($_FILES['new_photo']['tmp_name']);
...
//				fclose($file);
			...
			$data['picture_account'] = ''; //edit  - BLOB
